...
대시보드 앱은 분석 시스템의 API를 통해 관심종목의 **최신 분석결과**와 **스케줄러가 축적한 이전 분석 이력**을 조회한다. 최신 조회 API는 저장된 최신 행을 반환하며, 분석결과가 없는 경우에만 분석 시스템 내부에서 금융 데이터 조회, 프롬프트 구성, AI 에이전트 분석, JSON 저장을 수행한 뒤 새 분석결과를 반환한다. 이력 조회 API는 DB에 저장된 과거 행만 읽으며 재분석하지 않는다.
...
sequenceDiagram
    ...
    분석시스템-->>대시보드앱: 최신 분석결과 응답
    대시보드앱->>분석시스템: 분석 이력 목록 요청
    분석시스템-->>대시보드앱: 저장된 이력 응답
    opt 과거 시점 상세
        대시보드앱->>분석시스템: 특정 분석 id 상세 요청
        분석시스템-->>대시보드앱: 과거 분석 상세 응답
    end
    대시보드앱-->>투자자: 최신·이전 분석자료 표시
```
